...
func (s *Service) ClockRequest(req *cosi.ClockRequest) (*cosi.ClockResponse, onet.ClientError) {
	...
	if tree == nil {
		return nil, onet.NewClientErrorCode(cosi.ErrorParse, "couldn't create tree")
	}
	...
	start := time.Now()
	pi.Start()
	resp := &cosi.ClockResponse{
		Signature: <-pi.(*protocol.CosiRootNode).FinalSignature,
	}
...
func (s *Service) CountRequest(req *cosi.CountRequest) (*cosi.CountResponse, onet.ClientError) {
	s.storage.Lock()
	defer s.storage.Unlock()
	return &cosi.CountResponse{Count: s.storage.Count}, nil
}
